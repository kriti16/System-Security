...
int http_read_line(int fd, char *buf, size_t max_buf_len)
{
   size_t i = 0;
...
const char *http_request_line(int fd, char *reqpath, size_t reqpath_len, char *env, size_t *env_len, size_t max_env_len)
{
    char buf[4096];
    ...
    int env_len_now = 0;
    int ret_read_line = http_read_line(fd,buf,sizeof(buf));
    if (ret_read_line == -1)
        return "Socket IO error";

    /* For extra large buffer*/
    if (ret_read_line == -2)
    ...
    *sp2 = '\0';
    ...
    env_len_now = strlen(buf) + strlen("REQUEST_METHOD=");
    if(env_len_now > max_env_len || env_len_now < 0)
        return "Request too large";
    envp += sprintf(envp, "REQUEST_METHOD=%s", buf) + 1;
    env_len_now += (strlen("SERVER_PROTOCOL=")+strlen(sp2));
    if(env_len_now > max_env_len || env_len_now < 0)
        return "Request too large";
    ...
    {
        *qp = '\0';
        env_len_now += (strlen("QUERY_STRING=")+strlen(qp+1));
        if(env_len_now > max_env_len || env_len_now < 0)
            return "Request too large";
    ...
    env_len_now += (strlen("REQUEST_URI=")+strlen(reqpath));
    if(env_len_now > max_env_len || env_len_now < 0)
        return "Request too large";
    envp += sprintf(envp, "REQUEST_URI=%s", reqpath) + 1;

    env_len_now += strlen("SERVER_NAME=zoobar.org");
    if(env_len_now > max_env_len || env_len_now < 0)
        return "Request too large";
...
void http_serve(int fd, const char *name)
{
    ...
    if((sizeof(pn) - strlen(pn)) <= strlen(name)){
        http_err(fd, 500, "http_serve: %s","File path too long");
        return;
    }
...
void http_serve_file(int fd, const char *pn)
{
    ...
    if (getenv("PATH_INFO")) {
        ...
        char buf[1024];
        if(strlen(pn) + strlen(getenv("PATH_INFO")) > sizeof(buf)){
            http_err(fd, 500, "PATH_INFO too long");
            return;
        }
