...
{
    // printf("Launching Service ... by me\n");
    int fds[2], i;
    pid_t pid;
    ...
    {
        /* chroot into dir */
        printf("%s chdir ret:%d\n",name,chdir(dir));
        printf("%s chroot ret:%d\n",name,chroot(dir));
    }

    if (NCONF_get_number_e(conf, name, "gid", &gid))
    {
        /* change real, effective, and saved gid to gid */
        printf("%s setresgid %ld ret:%d\n",name,gid,setresgid(gid,gid,gid));
        warnx("setgid %ld", gid);
    }
    ...
    if (NCONF_get_number_e(conf, name, "uid", &uid))
    {
        ...
        printf("%s setresuid %ld ret:%d\n",name,uid,setresuid(uid,uid,uid));
        warnx("setuid %ld", uid);
    }
